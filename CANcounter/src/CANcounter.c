...
int main(void)
{
	...
	getCanSequence("IDsCaptured4.log");

	orderMessages();

	summaryOutput(outputFile);

	csvOutput("RDCANtiming4.csv");
...
void getCanSequence(char *filename)
{
	...
	while(fgets(inputStr, 190, bufferFile) != NULL)
	{
		...
		unsigned int scanReturn = sscanf(inputStr,"{=0x%3X=}",&ID);
		printf("%u Found ID: %s\n", scanReturn, inputStr);
		...
		if(scanReturn == 1)
		{
			...
			if(pointer != 9999)
			{
				if(CAN1Buffer[pointer].canID == 0)
				{
					...
					CAN1Buffer[pointer].messageOffset = offset;
					offset++;
				}
...
void summaryOutput(FILE *output)
{
	...
	sprintf(outputStr, 						"\n\n\n\n| CAN ID |  Total |  Offset |\n");
	sprintf(outputStr + strlen(outputStr), 	        "|        | (logs) |  (logs) |\n");
	...
	for(pointer = 0; pointer < BUFFERSIZE; pointer++)
	{
		...
		{
			sprintf(outputStr, "|  0x%03X | %6u | %7u |\n",
...
void csvOutput(char *filename)
{
	...
	else
	{
		...
		sprintf(csvStr, 						"\n\n\n\n,,CAN ID,Total,Offset\n");
		sprintf(csvStr + strlen(csvStr), 	        ",,,(logs),(us)\n");
...
/* Order logged messages by offset */
void orderMessages(void)
{
